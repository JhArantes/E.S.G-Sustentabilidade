...
-- Permitindo o Sequence NR_Cliente inserir dados Manualmente ou Automaticamente pelo Oracle
ALTER TABLE MC_CLIENTE MODIFY NR_CLIENTE GENERATED BY DEFAULT ON NULL AS IDENTITY;
...
-- -------------------------------------------------------------------------------
-- Exercicio 06
-- Tente atualizar o valor do Status Produto para "X" 
UPDATE MC_PRODUTO 
SET ST_PRODUTO = "X"
WHERE CD_PRODUTO = 1;

-- Output <> Saida
-- Error report - Error report - ORA-02290: check constraint (RM564212.MC_PRODUTO_CK_ST_PROD) violated
-- Mais detalhes em -> https://docs.oracle.com/error-help/db/ora-02290/

-- Não foi possivel alterar Status para valores que não sejam "A" ou "I" de acordo com a Constraint Check
-- Error -> Constraint CHECK Violada
...
-- -------------------------------------------------------------------------------
-- Exercicio 07
-- Confirme todas as transações pendentes
